...
const InvitesRespondedListItem = withRouter(
  observer(({ invite }) => (
    <li onClick={() => invitesStore.selectInviteResponded(invite)}>
      ...
      <Checkbox
        label=''
        checked={invitesStore.filmstripsRespondedIDs.includes(invite._id)}
      />
    </li>
  ))
)
        ...
        <ul>
          {UI.loadingWrapper(invitesStore.isFilmstripsRespondedLoading, () =>
            filteredInvites().map(invite => (
              <InvitesRespondedListItem key={invite._id} invite={invite} />
            ))
          )}
        </ul>
  ...
  ({ t, match, filmstripId, showShareInvite, setShowShareInvite }) => {
    ...
    const share = event => {
      ...
      try {
        ...
        console.log(file)
        if (file) {
          bodyWithFile = `${body}\n\n${t('InvitesResponded.email.fileMsg', {
            file
          })}`
        }
        Meteor.call(
        ...
        <DialogContent>
          <form>
            <TextField
              ...
              placeholder={t('InvitesResponded.email.placeholder.Email')}
            />
            <TextField
              ...
              placeholder={t('InvitesResponded.email.placeholder.Subject')}
            />
            {/* TODO i18n */}
            <TextField
              ...
              fullwidth
              rows={10}
              placeholder={t('InvitesResponded.email.placeholder.Body', {
                username: Meteor.user().username || 'Your Filmstrip user'
              })}
            />
            <div>
              <ReactFilestack
