...
export class ResponseAnswer extends React.Component {
    constructor(props) {
        ...
        const serializedCachedState = localStorage.getItem(
            this.props.currentFrame._id
        )
    ...
    updateLocalStorageState = state => {
        ...
        Meteor.defer(() => {
            localStorage.setItem(
                this.props.currentFrame._id,
                JSON.stringify(Object.assign({}, this.state, state))
            )
        })
    }
    ...
    render() {
        ...
        const addVideo = event => {
            ...
            const regex = new RegExp(/recordVideo/)
            if (regex.test(window.location.toString())) {
              this.props.history.goBack()
            }
            this.props.history.push(
                `/response/${currentFrame.filmstripId}/${currentFrame._id}/${emailBase64}/recordVideo`
            )
        }
