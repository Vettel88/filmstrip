...
import { CircularProgress, Grid, GridCell, TextField, Typography } from 'rmwc'
import { prepareResponseView } from './ResponseCommon.jsx'
import {
    ResponseSendConfirmation,
    ResponseVerifyConfirmation
} from '/imports/methods/Response.js'
...
const ResponseFinishContainer = ({
    ...
    const responseUrl = `/response/${filmstrip._id}/${
        ...
        <IconCard
            ...
            <Form fullWidth onSubmit={handleSubmit}>
                <TextField
                    ...
                    value={userEmail}
                    onChange={event =>
                        setUserEmail(event.target.value.trim().toLowerCase())
                    }
                    outlined
                />
                <Typography use='caption' tag='p'>
                    {t('Response.FinishedCopy')}
                </Typography>
                <Button
                    ...
                    disabled={
                        !isSending && userEmail && emailIsValid(userEmail)
                            ? false
                            : true
                    }
                />
            ...
            {loadingWrapper(loading, () => (
                <Grid>
                    ...
                    <GridCell desktop={6} tablet={6} phone={4}>
                        <IconCard
                            image='/icons8-checked.svg'
                            headline={t('Response.Confirmed')}
                            caption={t('Response.ConfirmedCopy', { email })}
                        />
                        <PaddedCard>
...
export const ResponseConfirm = withTranslation()(
    withTracker(({ match }) => {
        const confirmationKey =
            match && match.params && match.params.confirmationKey
                ? match.params.confirmationKey
                : null
        ...
        return {
            email: match.params.emailBase64
                ? atob(match.params.emailBase64)
                : '',
            createdFilmstripId,
            confirmationKey
        }
