...
const checkFilmstripOwner = ({ filmstrip, filmstripId }) => {
    const filmstripToInvestigate =
        filmstrip ||
        Filmstrips.findOne(filmstripId, { fields: { createdBy: 1 } })
...
Meteor.methods({
    ...
    'answer.sendConfirmation': async function({ filmstripId, email }) {
        ...
        const link =
            process.env.ROOT_URL + `/confirm/${filmstripId}/${emailBase64}`
...
Filmstrip.cloud`
    ...
    async 'filmstrip.invite.shareRespondedInvites'({
        email,
        filmstripId,
        inviteIDs,
        subject,
        ...
        const responses = Filmstrips.find({
            _id: {
                $in: inviteIDs
            }
        }).map(({ _id }) => `\n${getLink(_id)}`)
